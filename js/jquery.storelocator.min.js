...
(function(a){a.fn.storeLocator=function(b){var c=a.extend({mapDiv:"map",listDiv:"list",formID:"user-location",zoomLevel:12,pinColor:"fe7569",pinTextColor:"000000",storeLimit:26,distanceAlert:60,xmlLocation:"locations.xml",listColor1:"ffffff",listColor2:"eeeeee",bounceMarker:true,slideMap:true,modalWindow:false},b);
return this.each(function(){var l=a(this);if(c.modalWindow==true){l.wrap('<div id="overlay"><div id="modal-window"><div id="modal-content">');a("#modal-window").prepend('<div id="close-icon"></div>');
a("#overlay").hide();}if(c.slideMap==true){l.hide();}var k,n,o,h,e,f,i;var g=new Array();var m={};m.EarthRadiusInMiles=3956;m.EarthRadiusInKilometers=6367;
...
q(null);}});};}a(function(){a(document).on("submit","#"+c.formID,function(r){r.preventDefault();var s=a("#"+c.formID).serialize();s=s.replace("address=","");
if(s==""){alert("The input box was blank.");}else{var q=new j();var p=s;q.geocode(p,function(t){if(t!=null){n=t.latitude;o=t.longitude;d(n,o);}else{alert("ERROR! Unable to geocode address");
}});s=s.replace(" ","+");}});});function d(p,q){a(function(){a.ajax({type:"GET",url:c.xmlLocation,dataType:"xml",success:function(r){var s=0;a(r).find("marker").each(function(){var w=a(this).attr("name");
...
C=g[y][13];}var G={zoom:c.zoomLevel,center:new google.maps.LatLng(p,q),mapTypeId:google.maps.MapTypeId.ROADMAP};if(c.slideMap==true){l.slideDown();}if(c.modalWindow==true){a("#overlay").fadeIn();
a(document).on("click","#close-icon",function(){a("#overlay").hide();});a(document).on("click","#overlay",function(){a("#overlay").hide();});a(document).on("click","#modal-window",function(y){y.stopPropagation();
});a(document).keyup(function(y){if(y.keyCode==27){a("#overlay").hide();}});}var P=new google.maps.Map(document.getElementById(c.mapDiv),G);var O=new Array();
var B=new google.maps.InfoWindow();if((g.length-1)<c.storeLimit-1){i=g.length-1;}else{i=c.storeLimit-1;}for(var A=0;A<=i;A++){var Q=String.fromCharCode("A".charCodeAt(0)+A);
var L=new google.maps.LatLng(g[A][2],g[A][3]);h=t(L,g[A][1],g[A][4],Q);O[A]=h;K(A);v(h,J,H,F,w,M,u,N,x,E,D,C);}a("#"+c.listDiv).empty();a(O).each(function(y,R){var S=String.fromCharCode("A".charCodeAt(0)+y);
