...
;(function ($, window, document, undefined){
	...
	var $this = $(this);
	var listTemplate, infowindowTemplate, dataTypeRead, originalData, originalZoom, userInput, olat, olng, storeNum;
	 ...
	 * @returns {{location: *[]}}
	 */
	function define_location_data(currentMarker, storeStart, page){
			var indicator = "";
	 ...
	 * @param page {number} optional current page
	 */
	function listClick(marker, storeStart, page){
			//Define the location data
			var locations = define_location_data(marker, storeStart, page);
	...
	//Infowindows
	function create_infowindow(marker, location, infowindow, storeStart, page){
					...
					originalDataRequest.then(function () {
							...
							$(function () {
									...
									if ((settings.fullMapStart === true && firstRun === true) || (settings.mapSettings.zoom === 0)) {
											myOptions = settings.mapSettings;
									...
									for (var y = 0; y <= storeNumToShow - 1; y++) {
											...
											create_infowindow(marker, null, infowindow, storeStart, page);
									}
									...
									$(markers).each(function (x, marker) {
											...
											listClick(currentMarker, storeStart, page);
									});
									...
									$(document).on('click.' + prefix, settings.listDiv + ' li', function () {
											...
											if (settings.bounceMarker === true) {
													...
													setTimeout(function () {
																	...
																	create_infowindow(selectedMarker, listLoc, infowindow, storeStart, page);
															}, 700
													);
											}
											else {
